...
constructor TPutThread.Create(YourFTPClient:TIdFTP; ASource,ADest:String; IsDir:boolean);
begin
  xSource:=ASource;
  xDest:=ADest;
...
begin
  ...
  FormGlavnoOkno.AfterPutThread('',IdFTPx);
  Terminate;
end;

procedure TPutThread.PutDirectory(IdFTPx:TIdFTP; ASourceDir:String; ADestDir:String);
...
function TPutThread.IncludeTrailingSlash(path:string):string;
begin
  //
